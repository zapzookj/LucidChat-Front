...
const ChatPage = () => {
  ...
  const heroineToStats = useCallback((h) => (h ? {
    intimacy:    h.statIntimacy    ?? 0,
    affection:   h.statAffection   ?? 0,
    dependency:  h.statDependency  ?? 0,
    playfulness: h.statPlayfulness ?? 0,
    trust:       h.statTrust       ?? 0,
    lust:        h.statLust        ?? 0,
    corruption:  h.statCorruption  ?? 0,
    obsession:   h.statObsession   ?? 0,
  } : null), []);
  ...
  const v2DerivedRoomInfo = useMemo(() => {
    ...
    return {
      ...
      defaultOutfit: null,
      chatMode: "STORY",
      statusLevel: currentSpeakerHeroine.statusLevel || "STRANGER",
      ...
      try {
        ...
        if (firstHeroine) {
          setRoomInfo({
            ...
            statusLevel: firstHeroine.statusLevel || "STRANGER",
          ...
          setCharacterStats(heroineToStats(firstHeroine));
                    ...
                    <section>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="relative">
                                ...
                                {/* [블록 B] 방 페르소나 = 시작 시점 프로필 스냅샷(모드 무관 read-only) — 편집은 보관함 > 페르소나 */}
                                <div className="w-full min-h-[8rem] bg-white/[0.03] border border-white/10 rounded-lg px-4 py-3 text-white/80 leading-relaxed whitespace-pre-wrap">
                                  {roomPersona ? (
                                    ...
                                    <span className="text-white/30 italic">(시작할 때의 프로필 소개가 비어 있었어요)</span>
                                  )}
                                </div>
                                <p className="mt-1.5 text-[10px] text-white/25">
                                  프로필을 바꾸면 새 이야기부터 반영됩니다. (보관함 › 페르소나)
                                </p>
                            </div>
